...
const PAGES = [
  ...
  { slug: "legal", path: "/servicios-legales" },
];
...
async function capturePage(browser, pageInfo, viewport) {
  ...
  const prefix = `${pageInfo.slug}-${viewport.name}`;
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    for (const p of PAGES) {
      const logs = await capturePage(browser, p, vp);
      allLogs[`${p.slug}-${vp.name}`] = logs;
    }
  }
